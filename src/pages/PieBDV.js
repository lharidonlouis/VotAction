...
function PieBDV({ data, num_tour, height }) {
  ...
  const totalVoix1 = filteredData1.reduce((sum, item) => sum + item.voix, 0);
  const totalVoix2 = filteredData2.reduce((sum, item) => sum + item.voix, 0);

  // Create datasets for each series
  const dataset1 = {
    data: filteredData1.map(item => item.voix),
...
if (dataset2 && dataset1.data.length !== dataset2.data.length) {
    ...
    for (let i = 0; i < dataset1.data.length; i++) {
      const matchingItem = filteredData2.find(item => item.nom === filteredData1[i].nom);
      if (matchingItem) {
        adjustedData2.push(matchingItem.voix);
...
const labels = filteredData1.map(item => `${item.nom} - ${Object.keys(GroupedNuances).find(key => GroupedNuances[key].includes(item.codnua))}`);
